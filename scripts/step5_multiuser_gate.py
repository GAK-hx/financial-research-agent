    ...
    response = await client.post(
        ...
        headers={
            **request_headers(case),
            "Idempotency-Key": f"step5-{case['case_id']}-{uuid4().hex}",
        },
...
async def main() -> None:
    ...
    target = Path(os.environ.get("ARTIFACTS_ROOT", "/artifacts")) / "step5"
    target.mkdir(parents=True, exist_ok=True)
    (target / "multiuser_gate.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (target / "MULTIUSER_GATE.md").write_text(
