...
TIMEOUT_SECONDS = int(os.environ.get("STEP8_FLASH_TIMEOUT_SECONDS", "900"))
ARTIFACTS_ROOT = Path(os.environ.get("ARTIFACTS_ROOT", "artifacts"))
ARTIFACT_BASENAME = os.environ.get(
    "STEP8_FLASH_ARTIFACT_BASENAME", "flash_e2e_gate"
)
CASES = (
    {
...
async def run_case(client: httpx.AsyncClient, case: dict) -> dict:
    ...
    response = await client.post(
        "/runs",
        headers=headers(case, f"step8-flash-{case['case_id']}-{uuid4().hex}"),
...
async def main() -> None:
    selected = {
        item.strip()
        for item in os.environ.get("STEP8_FLASH_CASES", "").split(",")
        if item.strip()
    }
    cases = tuple(
    ...
    if not cases:
        raise SystemExit("STEP8_FLASH_CASES did not match a configured case")
    ...
    lines = [
        "# Step 08.3 DeepSeek V4 Flash 端到端 Gate",
        "",
