...
TIMEOUT_SECONDS = float(os.environ.get("GATEWAY_VALIDATION_TIMEOUT_SECONDS", "30"))
ARTIFACTS_ROOT = Path(os.environ.get("ARTIFACTS_ROOT", "artifacts"))

# Validation-only credentials. They are intentionally non-secret and must match
# the temporary GATEWAY_API_CLIENTS value used for the integration run.
CASES = (
    {
        "case_id": "tenant-a-analyst-1",
        "credential": "gateway-key-a1",
        "tenant_id": "gateway-tenant-a",
        "user_id": "analyst-1",
        "stock_codes": ["600519", "300750", "601318", "600036", "601166"],
    },
    {
        "case_id": "tenant-a-analyst-2",
        "credential": "gateway-key-a2",
        "tenant_id": "gateway-tenant-a",
        "user_id": "analyst-2",
...
async def main() -> None:
    gate_tag = f"gateway-validation-{uuid4().hex[:12]}"
    jobs = [
    ...
    (target / "gateway_multiuser.json").write_text(
    ...
    (target / "gateway_multiuser.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
