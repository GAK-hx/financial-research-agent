...
async def main() -> None:
    ...
    async def run_case(name: str, question: str, query: QuerySpec) -> dict:
        ...
        try:
            response = await provider.create_plan_response(
                ...
                schemas,
                planning_context={
                    "validation": "model_online_planner",
    ...
    target = Path(settings.artifacts_root) / "model_validation"
    target.mkdir(parents=True, exist_ok=True)
    (target / "model_online.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    lines = [
        "# DeepSeek Flash Planner 并发验证",
        "",
    ...
    (target / "MODEL_ONLINE_VALIDATION.md").write_text(
