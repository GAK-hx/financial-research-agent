...
LEVELS = (1, 10, 30, 50)
AUTH_CREDENTIAL = "concurrency-load-credential"
...
def frozen_result(run_id: str, elapsed_ms: int) -> dict:
    return {
        ...
        "selected_skills": [],
        "skill_selection_reason": "Concurrent queue benchmark",
...
async def run_level(settings: Settings, level: int) -> dict:
    ...
    async with httpx.AsyncClient(
        transport=transport, base_url="http://concurrency-validation"
    ) as client:
...
async def main() -> None:
    ...
    target = Path(settings.artifacts_root) / "concurrency_validation"
