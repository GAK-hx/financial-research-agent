...
def _markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "## 边界说明",
            "",
            "本报告完成原始 API 响应到标准化事实的逐字段程序化对账。PDF 正文与第二人工复核不能由同一程序"
            "替代，保持为独立人工 Gate，不在此报告中伪造通过。",
            "",
            "## 失败样例",
            "",
            "```json",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit Step 01 raw-to-standardized data quality")
    parser.add_argument("--root", default="/artifacts/phase5_step01/quality_audit_v1")
    parser.add_argument(
        "--statement-root", default="/artifacts/phase5_step01/expansion_statements_v1"
    )
    parser.add_argument(
        "--pool-file",
        default="/artifacts/phase5_step01/expansion_pool_v1/expansion_pool_v1.json",
    )
