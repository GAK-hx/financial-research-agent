...
from financial_research_agent.risk.artifact_cache import RiskArtifactCache
...
async def run_gate(candidates: Path, output_root: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "gate": "phase5_step03_risk_artifact_cache",
        "execution_mode": "docker_frozen_response_semantic_gate",
        ...
        "checks": checks,
        "notes": [
            "This gate validates risk-domain cache semantics, not model quality.",
            "PostgreSQL/Redis/Spring restart and queue safety remain covered by Phase 4 Step 08.",
    ...
    lines = [
        "# 03-B 风险公共产物与多用户缓存 Gate",
        "",
        f"- 状态：`{report['status']}`；",
        "- 模式：Docker 内冻结响应语义测试，不调用模型、不报告准确率；",
        "- 范围：公共风险分析复用、分析 single-flight、用户展示隔离、增量失效；",
        "- 后端故障恢复与队列安全：沿用 Phase 4 Step 08 的 PostgreSQL/Redis/Spring Gate。",
        "",
        "## 1/5/20/50 用户结果",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Step 03-B risk cache gate")
    parser.add_argument("--candidates", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    args = parser.parse_args()
    result = asyncio.run(run_gate(args.candidates, args.output_root))
