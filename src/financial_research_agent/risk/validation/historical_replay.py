...
from financial_research_agent.risk.data.feature_pipeline import (
    FEATURE_VERSION,
    RULE_VERSION,
    _load_events,
    _load_visible_facts,
...
from financial_research_agent.risk.data.schema import ADS_CANDIDATE_SCHEMA, DWS_FEATURE_SCHEMA
...
def main() -> None:
    ...
    parser.add_argument("--root", type=Path, default=Path("/artifacts/runtime_validation/replay_v1"))
    parser.add_argument(
        "--statement-root",
        type=Path,
        default=Path("/artifacts/risk_data/expansion_statements_v1"),
    )
    parser.add_argument(
        "--disclosure-root",
        type=Path,
        default=Path("/artifacts/risk_data/disclosure_pool_v1"),
    )
    parser.add_argument(
        "--pool-file",
        type=Path,
        default=Path("/artifacts/risk_data/expansion_pool_v1/expansion_pool_v1.json"),
    )
