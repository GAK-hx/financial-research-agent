...
def build_gate(root: Path, *, artifacts_root: Path, docs_output: Path | None) -> dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build consolidated Step 02 Gate")
    parser.add_argument("--root", type=Path, default=Path("/artifacts/phase5_step02/final_gate_v1"))
    parser.add_argument(
        "--artifacts-root", type=Path, default=Path("/artifacts/phase5_step02")
    )
