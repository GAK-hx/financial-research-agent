...
def _markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- `FAIL`：至少一项自动 Gate 未通过；",
            "- 自有公司数据只用于业务演示和性能测试，不建立人工标签 Gate。",
        ]
    )
    return "\n".join(lines) + "\n"


def build_gate(root: Path, *, artifacts_root: Path) -> tuple[Path, Path]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the consolidated Step 01 Gate")
    parser.add_argument("--root", default="/artifacts/phase5_step01/final_gate_v1")
    parser.add_argument("--artifacts-root", default="/artifacts/phase5_step01")
    args = parser.parse_args()
    paths = build_gate(Path(args.root), artifacts_root=Path(args.artifacts_root))
