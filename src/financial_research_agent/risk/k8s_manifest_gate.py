...
def run_gate(root: Path, output_root: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "gate": "phase5_step03_k8s_manifest",
    ...
    (output_root / "manifest-gate.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    lines = [
        "# 03-D Kubernetes Manifest Gate",
        "",
        f"- 状态：`{report['status']}`；",
        "- 范围：Kustomize渲染、资源完整性、探针、资源限制、安全上下文和Secret边界；",
        "- 运行声明：本文件仅验证静态清单；集群实测结果见独立Runtime Gate；",
        "",
        "## 资源",
        "",
    ...
    (output_root / "manifest-gate.md").write_text("\n".join(lines), encoding="utf-8")
    return report


def main() -> None:
    parser = argparse.ArgumentParser(description="Validate Step 03-D K8s manifests")
    parser.add_argument("--root", type=Path, default=Path.cwd())
    parser.add_argument("--output-root", type=Path, required=True)
    args = parser.parse_args()
    report = run_gate(args.root.resolve(), args.output_root.resolve())
