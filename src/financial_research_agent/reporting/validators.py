...
def _claim_numbers(claim: str, stock_codes: list[str]) -> list[tuple[float, str, int]]:
    ...
    for token_match in NUMBER_PATTERN.finditer(numeric_claim):
        ...
        direction_context = numeric_claim[
            max(0, token_match.start() - 24) : token_match.start()
        ]
        direction_context = re.split(r"[，。；,;]", direction_context)[-1]
        if "回撤" not in direction_context:
...
class ReportValidator:
    def validate(
        ...
        summary_candidates = [
            number
            for item in summary_citations
            for number in _evidence_numbers(item)
        ]
        for value, unit, decimal_places in _claim_numbers(
            report.summary, query.stock_codes
        ):
            if not _number_supported(
                value, unit, decimal_places, summary_candidates
            ):
                errors.append(f"summary:NUMERIC_UNSUPPORTED:{value}{unit}")
        ...
        for index, claim in enumerate(report.claims):
            ...
            candidates = [number for item in cited for number in _evidence_numbers(item)]
            for value, unit, decimal_places in _claim_numbers(
                claim.claim, query.stock_codes
            ):
